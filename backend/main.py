...
async def process_voice(
    ...
    If a `target_file` (reference voice WAV) is provided, uses XTTS v2 to
    ...
    try:
        ...
        # XTTS path returns at cloner.sr (24 kHz); DSP fallback returns at 16 kHz.
        out_sr = cloner.sr

        transcript_out = ""
        if target_file is not None:
            # Save the uploaded target voice to a temp file for XTTS
            ...
            if os.path.isfile(profile_voice) and cloner.tts_engine is not None:
                ...
                if cloner.tts_engine is None:
                    print(f"[main] XTTS not available — using DSP pitch shift for {profile}.")
                else:
...
# ── 3. Text-to-Speech (XTTS Voice Cloning) ───────────────────────────
...
async def process_tts(
    ...
    """
    Synthesises speech from text using Coqui XTTS v2 voice cloning.
